#!/bin/bash
#
# I-JEPA 快速測試 - SLURM 作業腳本（家目錄版本）
#
#SBATCH --job-name=ijepa_test_home      # 顯示在squeue中的作業名稱
...
#SBATCH --error=ijepa_test_home.%J.err  # 錯誤日誌檔案
#SBATCH --output=ijepa_test_home.%J.out # 輸出日誌檔案

echo "================================================================"
echo "I-JEPA 測試作業開始（家目錄版本）"
...
echo "================================================================"
echo "創建測試資料（在家目錄）..."
echo "資料將儲存在: $HOME/ijepa_test_data/"
echo "================================================================"

# 創建測試資料
srun python3 create_test_data_home.py
...
# 執行I-JEPA測試
srun python3 main.py --fname configs/test_config_home.yaml --devices cuda:0
...
if [ $exit_code -eq 0 ]; then
    ...
    echo "SLURM輸出檔案: ijepa_test_home.$SLURM_JOB_ID.out"
else
    echo "✗ I-JEPA測試失敗"
    echo "請檢查錯誤日誌: ijepa_test_home.$SLURM_JOB_ID.err"
fi
...
# 清理詢問（可選）
echo "如需清理測試資料，請手動執行："
echo "rm -rf $HOME/ijepa_test_data"
