#!/bin/bash
#
# I-JEPA 快速測試 - SLURM 作業腳本 (A100版本)
#
#SBATCH --job-name=ijepa_test_a100      # 顯示在squeue中的作業名稱
#SBATCH --partition=a100_short-al9      # 使用A100 GPU短時間分區
#SBATCH --gres=gpu:1                    # 請求1個GPU
#SBATCH --time=6:00:00                  # 最大執行時間 6小時
#SBATCH --ntasks=1                      # 單一任務
#SBATCH --cpus-per-task=8               # 每個任務8個CPU核心
#SBATCH --mem=64G                       # A100需要更多記憶體
#SBATCH --error=ijepa_test_a100.%J.err  # 錯誤日誌檔案
#SBATCH --output=ijepa_test_a100.%J.out # 輸出日誌檔案

echo "================================================================"
echo "I-JEPA 測試作業開始 (A100版本)"
...
python3 -c "import torch; print(f'GPU名稱: {torch.cuda.get_device_name(0) if torch.cuda.is_available() else \"N/A\"}')"
...
echo "================================================================"
echo "開始I-JEPA訓練測試 (A100)..."
echo "配置: 1個epoch, batch_size=4, vit_tiny模型"
echo "A100 GPU應該能提供更快的執行速度"
echo "================================================================"
...
if [ $exit_code -eq 0 ]; then
    ...
    echo "SLURM輸出檔案: ijepa_test_a100.$SLURM_JOB_ID.out"
else
    echo "✗ I-JEPA測試失敗"
    echo "請檢查錯誤日誌: ijepa_test_a100.$SLURM_JOB_ID.err"
fi
